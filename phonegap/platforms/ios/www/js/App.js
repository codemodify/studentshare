
SSApp = {}

SSApp.HouseKeeping = {}
SSApp.HouseKeeping.FilterBorrow = false;
SSApp.HouseKeeping.FilterSell = false;
SSApp.HouseKeeping.FilterWanted = false;
SSApp.HouseKeeping.LastRepliesCount = 0;
SSApp.HouseKeeping.ServiceUrl = "http://mumstudentshare.herokuapp.com";


SSApp.Run = function () {
    SSApp.Forums.Setup();
}

SSApp.LoadingProgress = {}
SSApp.LoadingProgress.Show = function () {

    var dialog = jQuery("#App-LoadingProgress");
        dialog.kendoWindow({
            draggable: false,
            resizable: false,
            title: false,
            modal: true,
            actions: [],
            width: "100px",
            height: "100px"
        });
        dialog.data("kendoWindow").center();
        dialog.data("kendoWindow").open();
}
SSApp.LoadingProgress.Hide = function () {
    jQuery("#App-LoadingProgress").data("kendoWindow").close();
}

SSApp.Auth = {}
SSApp.Auth.UserId = "0000-0000-0000";
SSApp.Auth.User = "";
SSApp.Auth.Pass = "";
SSApp.Auth.Email = "";
SSApp.Auth.ActionId = -1;
SSApp.Auth.Setup = function (informAboutFunctionalities) {

    if (informAboutFunctionalities) {
        alert ("Guests are in 'read-only' mode.\n" +
               "To be able to do more you need to register.\n" +
               "Registration is easy fast & secure.");
    }

    jQuery("#App-AuthDialog-LoginButton").click(SSApp.Auth.LoginButtonClicked);
    jQuery("#App-AuthDialog-RegisterButton").click(SSApp.Auth.RegisterButtonClicked);

    var authDialog = jQuery("#App-AuthDialog");
        authDialog.kendoWindow({ 
            title: "Authentication Required",
            width: "270px",
            modal: true,
            resizable: false,            
            actions: ["Close"]
        });
        authDialog.data("kendoWindow").center();
        authDialog.data("kendoWindow").open();

    setTimeout(function () { jQuery("#App-AuthDialog-User").focus();}, 1000);
}
SSApp.Auth.LoginButtonClicked = function () {
    var user = jQuery("#App-AuthDialog-User").val();
    var pass = jQuery("#App-AuthDialog-Pass").val();

    if (user.trim().length === 0 || pass.trim().length === 0)
        return;

    var deviceRegistrationId = "000";
    var service = "http://" + window.location.host + "/LoginUser/" + user + "/" + pass + "/" + deviceRegistrationId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function(user) {
            if (user !== null) {

                SSApp.Auth.UserId = user.Id;
                SSApp.Auth.User = user.Name;
                SSApp.Auth.Pass = user.Pass;
                SSApp.Auth.Email = user.Email;

                jQuery("#App-Content-Left-Welcome").html("Hi, " + SSApp.Auth.User);
                jQuery("#App-AuthDialog").data("kendoWindow").close();
                if (SSApp.Auth.ActionId !== -1)
                    SSApp.Forums.LoadRouter();
                else
                    SSApp.Forums.Setup();
            }
            else {
                alert("Sorry no such user. You'll have to register.");
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}
SSApp.Auth.RegisterButtonClicked = function () {
    var user = jQuery("#App-AuthDialog-User").val();
    var pass = jQuery("#App-AuthDialog-Pass").val();

    if (user.trim().length === 0 || pass.trim().length === 0)
        return;

    var deviceRegistrationId = "000";
    var service = "http://" + window.location.host + "/RegisterUser/" + user + "/" + pass + "/" + deviceRegistrationId;
    
    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function (userId) {

            if (userId === "exists") {
                alert("User already exists. Pick another one.");
                return;
            }

            if (userId.length > 0) {

                SSApp.Auth.UserId = userId;
                SSApp.Auth.User = user;
                SSApp.Auth.Pass = pass;

                jQuery("#App-Content-Left-Welcome").html("Hi, " + SSApp.Auth.User);
                jQuery("#App-AuthDialog").data("kendoWindow").close();
                if (SSApp.Auth.ActionId !== -1)
                    SSApp.Forums.LoadRouter();
                else
                    SSApp.Forums.Setup();
            }
            else {
                alert("Sorry no such user. You'll have to register.");
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}

SSApp.Forums = {}

SSApp.Forums.DataSourceBackend = new kendo.data.ObservableArray([]);
SSApp.Forums.DataSource = new kendo.data.DataSource({ data: SSApp.Forums.DataSourceBackend, pageSize: 12 });

SSApp.Forums.ChatLastId = "0000-0000-0000";
SSApp.Forums.ChatDataSourceBackend = new kendo.data.ObservableArray([]);
SSApp.Forums.ChatDataSource = new kendo.data.DataSource({ data: SSApp.Forums.ChatDataSourceBackend });

SSApp.Forums.Setup = function () {

    jQuery("#App-Content-Left").css("visibility", "visible");
    jQuery("#App-Content-Right").css("visibility", "visible");

    jQuery("#panelbar").kendoPanelBar({
        expandMode: "single",
        select: SSApp.Forums.LoadRouter
    });

    var dialog = jQuery("#App-Content-Right");
        dialog.kendoWindow({ 
            title: "Choose a forum",
            width: "500px",
            height: "576px",
            resizable: false,
            actions: ["New"]
        });
        dialog.data("kendoWindow").open();
        dialog.closest(".k-window").css({
            top: 30,
            left: 370,
        });
        dialog.data("kendoWindow").wrapper.find(".k-i-new").click( function (e) {
            SSApp.Forums.AddNewRouter();
            e.preventDefault();
        });

    jQuery("#pager").kendoPager({
        dataSource: SSApp.Forums.DataSource
    });

    jQuery("#listView").kendoListView({
        dataSource: SSApp.Forums.DataSource,
        template: kendo.template($("#template").html())
    });

    var options = {
        show_labels: true,
        labels_placement: "both",
        on_label: "Me&nbsp;",
        off_label: "all",
        width: 40,
        height: 20,
        button_width: 20
    };
    jQuery("#borrowMy").switchButton(options).change(function (e) {
        SSApp.HouseKeeping.FilterBorrow = jQuery("#borrowMy").prop("checked");
        SSApp.Forums.LoadBorrow();
    });
    jQuery("#sellMy").switchButton(options).change(function (e) {
        SSApp.HouseKeeping.FilterSell = jQuery("#sellMy").prop("checked");
        SSApp.Forums.LoadSell();
    });
    jQuery("#wantedMy").switchButton(options).change(function (e) {
        SSApp.HouseKeeping.FilterWanted = jQuery("#wantedMy").prop("checked");
        SSApp.Forums.LoadSearch();
    });

    if (SSApp.Auth.UserId !== "0000-0000-0000") {
        setTimeout(SSApp.Forums.PullNotifications, 3000);
    }
}
SSApp.Forums.PullNotifications = function () {

    var service = "http://" + window.location.host + "/GetInterestedUsers/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function (count) {

            if (SSApp.HouseKeeping.LastRepliesCount !== count) {

                jQuery("#App-Content-Left-Forums-Replies").html(count);

                SSApp.HouseKeeping.LastRepliesCount = count;

                ////////jQuery("#App-Content-Left-Forums-Replies").css("color", "red");
                ////////jQuery("#App-Content-Left-Forums-Replies").css("font-weight", "bold");

                jQuery("#App-Content-Left-Forums-Replies").kendoTooltip({
                    content: count,
                    position: "top"
                }).show();

                var tooltip = jQuery("#App-Content-Left-Forums-Replies").kendoTooltip({
                    showOn: "click",
                    autoHide: true,
                    position: "bottom",
                    content: "You have new feedback"
                });
                tooltip.data("kendoTooltip").show(jQuery("#App-Content-Left-Forums-Replies"));
            }
            else {
                //jQuery("#App-Content-Left-Forums-Replies").css("color", "black");
                //jQuery("#App-Content-Left-Forums-Replies").css("font-weight", "normal");
            }
            
            setTimeout(SSApp.Forums.PullNotifications, 5000);
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}

SSApp.Forums.LoadRouter = function (e) {

    var text = $(e.item).find("> .k-link").text().trim();

    if (text.indexOf("Chat !") !== -1 ) {
        SSApp.Forums.LoadChat();
        return;
    }

    text = text.replace("allMe","").trim();
    jQuery("#App-Content-Right").data("kendoWindow").title(text);

    if (text.indexOf("Borrow") !== -1 ) {
        SSApp.Auth.ActionId = 0;
        SSApp.Forums.LoadBorrow();
    }

    else if (text.indexOf("Get Rid (sell)") !== -1 ) {
        SSApp.Auth.ActionId = 1;
        SSApp.Forums.LoadSell();
    }

    else if (text.indexOf("Wanted (buy)") !== -1) {
        SSApp.Auth.ActionId = 2;
        SSApp.Forums.LoadSearch();
    }
}
SSApp.Forums.LoadBorrow = function () {
    SSApp.LoadingProgress.Show();
    
    var service = "http://" + window.location.host + "/LoadBorrow/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function(data) {

            while (SSApp.Forums.DataSourceBackend.length > 0)
                SSApp.Forums.DataSourceBackend.pop();

            if (SSApp.HouseKeeping.FilterBorrow) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].OwnerId === SSApp.Auth.UserId) {
                        data[i].CanDelete = (data[i].OwnerId === SSApp.Auth.UserId) ? "visibleElement" : "hiddenElement";
                        SSApp.Forums.DataSourceBackend.push(data[i]);
                    }
                }
            }
            else {
                for (var i = 0; i < data.length; i++) {
                    data[i].CanDelete = (data[i].OwnerId === SSApp.Auth.UserId) ? "visibleElement" : "hiddenElement";
                    SSApp.Forums.DataSourceBackend.push(data[i]);
                }
            }
            
            SSApp.LoadingProgress.Hide();            
        },
        error: function(jqXHR, textStatus, errorThrown) {
            SSApp.LoadingProgress.Hide();        
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}
SSApp.Forums.LoadSell = function () {
    SSApp.LoadingProgress.Show();

    var service = "http://" + window.location.host + "/LoadSell/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function(data) {

            while (SSApp.Forums.DataSourceBackend.length > 0)
                SSApp.Forums.DataSourceBackend.pop();

            if (SSApp.HouseKeeping.FilterSell) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].OwnerId === SSApp.Auth.UserId) {
                        data[i].CanDelete = (data[i].OwnerId === SSApp.Auth.UserId) ? "visibleElement" : "hiddenElement";
                        SSApp.Forums.DataSourceBackend.push(data[i]);
                    }
                }
            }
            else {
                for (var i = 0; i < data.length; i++) {
                    data[i].CanDelete = (data[i].OwnerId === SSApp.Auth.UserId) ? "visibleElement" : "hiddenElement";
                    SSApp.Forums.DataSourceBackend.push(data[i]);
                }
            }
            
            SSApp.LoadingProgress.Hide();            
        },
        error: function(jqXHR, textStatus, errorThrown) {
            SSApp.LoadingProgress.Hide();
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}
SSApp.Forums.LoadSearch = function () {
    SSApp.LoadingProgress.Show();
    
    var service = "http://" + window.location.host + "/LoadSearch/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function(data) {

            while (SSApp.Forums.DataSourceBackend.length > 0)
                SSApp.Forums.DataSourceBackend.pop();

            if (SSApp.HouseKeeping.FilterWanted) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].OwnerId === SSApp.Auth.UserId) {
                        data[i].CanDelete = (data[i].OwnerId === SSApp.Auth.UserId) ? "visibleElement" : "hiddenElement";
                        SSApp.Forums.DataSourceBackend.push(data[i]);
                    }
                }
            }
            else {
                for (var i = 0; i < data.length; i++) {
                    data[i].CanDelete = (data[i].OwnerId === SSApp.Auth.UserId) ? "visibleElement" : "hiddenElement";
                    SSApp.Forums.DataSourceBackend.push(data[i]);
                }
            }

            SSApp.LoadingProgress.Hide();
        },
        error: function(jqXHR, textStatus, errorThrown) {
            SSApp.LoadingProgress.Hide();
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });    
}

SSApp.Forums.LoadChat = function () {

    var isVisible = jQuery("#App-Content-Right-Chat").css("visibility");
    if (isVisible !== "visible") {
        jQuery("#App-Content-Right-Chat").css("visibility", "visible");

        var dialog = jQuery("#App-Content-Right-Chat");
            dialog.kendoWindow({
                title: "Chat !",
                width: "300px",
                height: "400px",
                resizable: false,
                actions: ["ReloadMessages"]
            });
            dialog.data("kendoWindow").open();
            dialog.closest(".k-window").css({
                top: 70,
                left: 470,
            });
            dialog.data("kendoWindow").wrapper.find(".k-i-reloadmessages").click(function (e) {
                e.preventDefault();
            });

        jQuery("#App-Content-Right-Chat-ListView").kendoListView({
            dataSource: SSApp.Forums.ChatDataSource,
            template: kendo.template($("#App-Content-Right-Chat-Template").html())
        });

        jQuery("#App-Content-Right-Chat-SendButton").click(function (e) {

            e.preventDefault();

            if (SSApp.Auth.UserId === "" || SSApp.Auth.User === "" || SSApp.Auth.Pass === "") {
                SSApp.Auth.Setup(true);
                return;
            }

            var message = jQuery("#App-Content-Right-Chat-Message").val();
            if (message === "")
                return;

            jQuery("#App-Content-Right-Chat-Message").val("");

            var service = "http://" + window.location.host + "/ChatSendMessage"
                + "/" + SSApp.Auth.UserId
                + "/" + jQuery.base64.encode(message);

            jQuery.ajax({
                url: service,
                dataType: "json",
                cache: false,
                timeout: 5000,
                success: function (data) {

                    SSApp.Forums.LoadChatContent(false);
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    SSApp.LoadingProgress.Hide();
                    alert('Error: ' + textStatus + " " + errorThrown);
                }
            });
        });

        SSApp.Forums.LoadChatContent(true);
    }
    else {
        var dialog = jQuery("#App-Content-Right-Chat");
            dialog.data("kendoWindow").open();
    }
}
SSApp.Forums.LoadChatContent = function (repeat) {
    var service = "http://" + window.location.host + "/ChatGetMessages"
        + "/" + SSApp.Auth.UserId
        + "/" + SSApp.Forums.ChatLastId;

    var dialog = jQuery("#App-Content-Right-Chat");
        dialog.data("kendoWindow").wrapper.find(".k-i-reloadmessages").show();

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function(data) {

            //if (SSApp.Auth.UserId === "" || SSApp.Auth.User === "" || SSApp.Auth.Pass === "") {

            while (SSApp.Forums.ChatDataSourceBackend.length > 0)
                SSApp.Forums.ChatDataSourceBackend.pop();

            // }
            for (var i=0; i < data.length; i++) {
                SSApp.Forums.ChatDataSourceBackend.push(data[i]);
            }
            dialog.data("kendoWindow").wrapper.find(".k-i-reloadmessages").hide();

            //var listView = $("#App-Content-Right-Chat-ListView");
            //listView.scrollTop(listView[0].scrollHeight);

            if (repeat) {
                setTimeout(function () { SSApp.Forums.LoadChatContent(true); }, 2000);
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            dialog.data("kendoWindow").wrapper.find(".k-i-reloadmessages").hide();
            //////alert('Error: ' + textStatus + " " + errorThrown);

            if (repeat) {
                setTimeout(function () { SSApp.Forums.LoadChatContent(true); }, 2000);
            }
        }
    });
}

SSApp.Forums.AddNewRouter = function () {
    if (SSApp.Auth.UserId === "" || SSApp.Auth.User === "" || SSApp.Auth.Pass === "") {
        SSApp.Auth.Setup(true);
        return;
    }

    jQuery("#App-AddNew-Sell-AddButton").unbind("click");
    jQuery("#App-AddNew-Sell-CancelButton").unbind("click");
    jQuery("#App-AddNew-Sell-Type").kendoDropDownList();
    
    jQuery("#App-AddNew-Sell-CancelButton").click(function () {
        jQuery("#App-AddNew-Sell").data("kendoWindow").close();
    });

    var title = "";

    if (SSApp.Auth.ActionId === 0) {
        title = "Borrow";
        SSApp.Forums.AddNewBorrow();
    }
    else if (SSApp.Auth.ActionId === 1) {
        title = "Get Rid Of";
        SSApp.Forums.AddNewSell();
    }
    else if (SSApp.Auth.ActionId === 2) {
        title = "Wanted";
        SSApp.Forums.AddNewSearch();
    }
    else if (SSApp.Auth.ActionId === 3) {
        SSApp.Forums.AddNewChat();
        return;
    }
    
    var dialog = jQuery("#App-AddNew-Sell");
        dialog.kendoWindow({ 
            title: title,
            width: "300px",
            modal: true,
            resizable: false,
            actions: []
        });
        dialog.data("kendoWindow").title(title);
        dialog.data("kendoWindow").center();
        dialog.data("kendoWindow").open();
}
SSApp.Forums.AddNewBorrow = function (id) {
    jQuery("#App-AddNew-Sell-AddButton").click(function () {

        var type  = jQuery("#App-AddNew-Sell-Type").val();
        var phone = jQuery("#App-AddNew-Sell-Phone").val();
        var email = jQuery("#App-AddNew-Sell-Email").val();
        var desc  = jQuery("#App-AddNew-Sell-Description").val();
        var price = jQuery("#App-AddNew-Sell-Price").val();

        if (phone.trim().length === 0
            && email.trim().length === 0
            && desc.trim().length === 0
            && price.trim().length === 0)
            return;

            type  = type.length  === 0 ? " " : type;
            phone = phone.length === 0 ? " " : phone;
            email = email.length === 0 ? " " : email;
            desc  = desc.length  === 0 ? " " : desc;
            price = price.length === 0 ? " " : price;

        var service = "http://" + window.location.host + "/AddBorrow" 
            + "/" + SSApp.Auth.UserId
            + "/" + jQuery.base64.encode(type)
            + "/" + jQuery.base64.encode(phone)
            + "/" + jQuery.base64.encode(email.replace(".", " . ").replace("@", " @ "))
            + "/" + jQuery.base64.encode(desc)
            + "/" + jQuery.base64.encode(price);

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function(sellId) {
                if (sellId.length > 0) {
                    jQuery("#App-AddNew-Sell").data("kendoWindow").close();
                }
                else {
                    alert("Sorry some error happened while adding.");
                }
                    
                SSApp.Forums.LoadBorrow();
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    });
}
SSApp.Forums.AddNewSell = function (id) {
    jQuery("#App-AddNew-Sell-AddButton").click(function () {

        var type  = jQuery("#App-AddNew-Sell-Type").val();
        var phone = jQuery("#App-AddNew-Sell-Phone").val();
        var email = jQuery("#App-AddNew-Sell-Email").val();
        var desc  = jQuery("#App-AddNew-Sell-Description").val();
        var price = jQuery("#App-AddNew-Sell-Price").val();
            
        if (phone.trim().length === 0
            && email.trim().length === 0
            && desc.trim().length === 0
            && price.trim().length === 0)
            return;

            type  = type.length  === 0 ? " " : type;
            phone = phone.length === 0 ? " " : phone;
            email = email.length === 0 ? " " : email;
            desc  = desc.length  === 0 ? " " : desc;
            price = price.length === 0 ? " " : price;

        var service = "http://" + window.location.host + "/AddSell" 
            + "/" + SSApp.Auth.UserId
            + "/" + jQuery.base64.encode(type)
            + "/" + jQuery.base64.encode(phone)
            + "/" + jQuery.base64.encode(email.replace(".", " . ").replace("@", " @ "))
            + "/" + jQuery.base64.encode(desc)
            + "/" + jQuery.base64.encode(price);

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function(sellId) {
                if (sellId.length > 0) {
                    jQuery("#App-AddNew-Sell").data("kendoWindow").close();
                }
                else {
                    alert("Sorry some error happened while adding.");
                }
                    
                SSApp.Forums.LoadSell();
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    });
}
SSApp.Forums.AddNewSearch = function (id) {
    jQuery("#App-AddNew-Sell-AddButton").click(function () {

        var type  = jQuery("#App-AddNew-Sell-Type").val();
        var phone = jQuery("#App-AddNew-Sell-Phone").val();
        var email = jQuery("#App-AddNew-Sell-Email").val();
        var desc  = jQuery("#App-AddNew-Sell-Description").val();
        var price = jQuery("#App-AddNew-Sell-Price").val();
            
        if (phone.trim().length === 0
            && email.trim().length === 0
            && desc.trim().length === 0
            && price.trim().length === 0)
            return;

            type  = type.length  === 0 ? " " : type;
            phone = phone.length === 0 ? " " : phone;
            email = email.length === 0 ? " " : email;
            desc  = desc.length  === 0 ? " " : desc;
            price = price.length === 0 ? " " : price;

        var service = "http://" + window.location.host + "/AddSearch" 
            + "/" + SSApp.Auth.UserId
            + "/" + jQuery.base64.encode(type)
            + "/" + jQuery.base64.encode(phone)
            + "/" + jQuery.base64.encode(email.replace(".", " . ").replace("@", " @ "))
            + "/" + jQuery.base64.encode(desc)
            + "/" + jQuery.base64.encode(price);

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function(sellId) {
                if (sellId.length > 0) {
                    jQuery("#App-AddNew-Sell").data("kendoWindow").close();
                }
                else {
                    alert("Sorry some error happened while adding.");
                }
                    
                SSApp.Forums.LoadSearch();
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    });
}

SSApp.Forums.RemoveRouter = function (id) {
    if (SSApp.Auth.UserId === "" || SSApp.Auth.User === "" || SSApp.Auth.Pass === "") {
        SSApp.Auth.Setup(true);
        return;
    }

    var confirmation = confirm("Sure To Delete ?");
    if (confirmation !== true)
        return;

    if (SSApp.Auth.ActionId === 0)
        SSApp.Forums.RemoveBorrow(id);

    else if (SSApp.Auth.ActionId === 1)
        SSApp.Forums.RemoveSell(id);

    else if (SSApp.Auth.ActionId === 2)
        SSApp.Forums.RemoveSearch(id);
}
SSApp.Forums.RemoveBorrow = function (id) {

    var service = "http://" + window.location.host + "/RemoveBorrow" 
        + "/" + id
        + "/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function(data) {
            SSApp.Forums.LoadBorrow();
        },
        error: function(jqXHR, textStatus, errorThrown) {
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}
SSApp.Forums.RemoveSell = function (id) {
    var service = "http://" + window.location.host + "/RemoveSell"
        + "/" + id
        + "/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function (data) {
            SSApp.Forums.LoadSell();
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}
SSApp.Forums.RemoveSearch = function (id) {
    var service = "http://" + window.location.host + "/RemoveSearch"
        + "/" + id
        + "/" + SSApp.Auth.UserId;

    jQuery.ajax({
        url: service,
        dataType: "json",
        cache: false,
        timeout: 5000,
        success: function (data) {
            SSApp.Forums.LoadSearch();
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert('Error: ' + textStatus + " " + errorThrown);
        }
    });
}

SSApp.Forums.WantRouter = function (id) {
    if (SSApp.Auth.UserId === "" || SSApp.Auth.User === "" || SSApp.Auth.Pass === "") {
        SSApp.Auth.Setup(true);
        return;
    }

    if (SSApp.Auth.Email === "" || SSApp.Auth.Email === null || SSApp.Auth.Email === undefined) {
        var confirmation = confirm("When you WANT stuff, it is a good idea to set your email.\n" +
                                   "Not requried but this way the author could contact you.");
        if (confirmation === true) {
            jQuery("#App-SettingsDialog-OkButton").unbind("click");
            jQuery("#App-SettingsDialog-OkButton").click(function (e) {
                e.preventDefault();

                var email = jQuery("#App-SettingsDialog-Email").val();

                if (email.trim().length === 0) {
                    alert("Set email");
                    return;
                }

                var service = "http://" + window.location.host + "/UpdateUserProfile/" + SSApp.Auth.UserId + "/" + email;

                jQuery.ajax({
                    url: service,
                    dataType: "json",
                    cache: false,
                    timeout: 5000,
                    success: function (data) {

                        SSApp.Auth.Email = email;

                        var dialog = jQuery("#App-SettingsDialog");
                            dialog.data("kendoWindow").close();

                        if (data === false) {
                            alert("Error while updating your profile.");
                        }
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        var dialog = jQuery("#App-SettingsDialog");
                            dialog.data("kendoWindow").close();

                        alert('Error: ' + textStatus + " " + errorThrown);
                    }
                });
            });

            var dialog = jQuery("#App-SettingsDialog");
                dialog.kendoWindow({
                    title: "Profile Update",
                    width: "270px",
                    modal: true,
                    resizable: false,
                    actions: ["Close"]
                });
                dialog.data("kendoWindow").center();
                dialog.data("kendoWindow").open();

            setTimeout(function () { jQuery("#App-SettingsDialog-Email").focus(); }, 1000);
        }
    }

    SSApp.Forums.WantRouterHelper(id);
}
SSApp.Forums.WantRouterHelper = function (id) {
    if (SSApp.Auth.ActionId === 0)
        SSApp.Forums.WantBorrow(id);

    else if (SSApp.Auth.ActionId === 1)
        SSApp.Forums.WantSell(id);

    else if (SSApp.Auth.ActionId === 2)
        SSApp.Forums.WantSearch(id);
}
SSApp.Forums.WantBorrow = function (id) {

    var ownerId = "";

    for (var i = 0; i < SSApp.Forums.DataSourceBackend.length; i++) {
        if (SSApp.Forums.DataSourceBackend[i].Id === id) {
            ownerId = SSApp.Forums.DataSourceBackend[i].OwnerId;
            break;
        }
    }

    if (ownerId === SSApp.Auth.UserId) {
        var service = "http://" + window.location.host + "/GetInterestedUsersToBorrow/" + id;

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function (users) {
                var dialog = jQuery("#App-ShowInterestedUsers");
                    dialog.kendoWindow({
                        title: "Interested Users",
                        width: "300px",
                        height: "400px",
                        resizable: true,
                        actions: ["Close"]
                    });
                    dialog.data("kendoWindow").open();
                    dialog.closest(".k-window").css({
                        top: 70,
                        left: 470,
                    });

                var html = "<table>";
                for (var i = 0; i < users.length; i++) {
                    html = html + "<tr><td>" + users[i].Name + "</td><td> -> </td><td><a href='mailto:" + users[i].Email + "'>" + users[i].Email + "</td></tr>";
                }
                html = html + "</table>";
                dialog.html(html);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    }
    else {
        var service = "http://" + window.location.host + "/WantBorrow"
            + "/" + id
            + "/" + SSApp.Auth.UserId;

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function (data) {
                SSApp.Forums.LoadBorrow();
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    }
}
SSApp.Forums.WantSell = function (id) {

    var ownerId = "";

    for (var i = 0; i < SSApp.Forums.DataSourceBackend.length; i++) {
        if (SSApp.Forums.DataSourceBackend[i].Id === id) {
            ownerId = SSApp.Forums.DataSourceBackend[i].OwnerId;
            break;
        }
    }

    if (ownerId === SSApp.Auth.UserId) {
        var service = "http://" + window.location.host + "/GetInterestedUsersToSell/" + id;

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function (users) {
                var dialog = jQuery("#App-ShowInterestedUsers");
                    dialog.kendoWindow({
                        title: "Interested Users",
                        width: "300px",
                        height: "400px",
                        resizable: true,
                        actions: ["Close"]
                    });
                    dialog.data("kendoWindow").open();
                    dialog.closest(".k-window").css({
                        top: 70,
                        left: 470,
                    });

                var html = "<table>";
                for (var i = 0; i < users.length; i++) {
                    html = html + "<tr><td>" + users[i].Name + "</td><td> -> </td><td><a href='mailto:" + users[i].Email + "'>" + users[i].Email + "</td></tr>";
                }
                html = html + "</table>";
                dialog.html(html);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    }
    else {
        var service = "http://" + window.location.host + "/WantSell"
            + "/" + id
            + "/" + SSApp.Auth.UserId;

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function (data) {
                SSApp.Forums.LoadSell();
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    }
}
SSApp.Forums.WantSearch = function (id) {

    var ownerId = "";

    for (var i = 0; i < SSApp.Forums.DataSourceBackend.length; i++) {
        if (SSApp.Forums.DataSourceBackend[i].Id === id) {
            ownerId = SSApp.Forums.DataSourceBackend[i].OwnerId;
            break;
        }
    }

    if (ownerId === SSApp.Auth.UserId) {
        var service = "http://" + window.location.host + "/GetInterestedUsersToSearch/" + id;

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function (users) {
                var dialog = jQuery("#App-ShowInterestedUsers");
                    dialog.kendoWindow({
                        title: "Interested Users",
                        width: "300px",
                        height: "400px",
                        resizable: true,
                        actions: ["Close"]
                    });
                    dialog.data("kendoWindow").open();
                    dialog.closest(".k-window").css({
                        top: 70,
                        left: 470,
                    });

                var html = "<table>";
                for (var i = 0; i < users.length; i++) {
                    html = html + "<tr><td>" + users[i].Name + "</td><td> -> </td><td><a href='mailto:" + users[i].Email + "'>" + users[i].Email + "</td></tr>";
                }
                html = html + "</table>";
                dialog.html(html);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    }
    else {
        var service = "http://" + window.location.host + "/WantSearch"
            + "/" + id
            + "/" + SSApp.Auth.UserId;

        jQuery.ajax({
            url: service,
            dataType: "json",
            cache: false,
            timeout: 5000,
            success: function (data) {
                SSApp.Forums.LoadSearch();
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + " " + errorThrown);
            }
        });
    }
}
